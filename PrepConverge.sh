for M in ketene ethylene glyoxal butadiene benzene ; do # benzoquinone
    echo ${M}
    ./Invert-Lieb.py -M ./SysDB/${M}.xyz --CCSD --Basis cc-pvdz --CalcPot > Outputs/${M}_cc-pvdz.out
...
for B in cc-pvdz cc-pvtz cc-pvqz; do
	...
	echo "${M} @ ${B}"
	./Invert-Lieb.py -M ./SysDB/${M}.xyz --CCSD --Basis ${B} --NIter 500 \
			 --En_Exit 1e-8 --CalcPot > Outputs/${M}_${B}.out
